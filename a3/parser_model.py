...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden = nn.Linear(self.embed_size * self.n_features, self.hidden_size) #输入维度是特征数乘嵌入维度
        nn.init.xavier_uniform_(self.embed_to_hidden.weight,gain=1)
        self.drouput = nn.Dropout(self.dropout_prob)
    ...
    def embedding_lookup(self, t):
        ...
        x = self.pretrained_embeddings(t) #得到的x的维度为(batch_size,n_features,embedding_size)
        x = x.view(t.size(0),-1)  #将x的形状改为(batch_size,n_feature * embedding_size)
        ### END YOUR CODE
        return x
